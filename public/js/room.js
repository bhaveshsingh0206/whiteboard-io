	...
	document.querySelectorAll('.room-list li').forEach(element => {
		...
		window.rooms.forEach((room)=>{
			if (room.roomName === selectedElement.textContent) {
				$('.room-language').html('<img src="../img/'+room.language+'.png" alt="" srcset="" />')
				$('#room-header').text(room.roomName)
				$('#room-description').text('Created By : '+room.username)
				$('#room-description').attr({'roomId':room.roomId,'username':room.username});
...
window.setInterval(()=> {
		...
		socket.on('send-rooms', (data) => {
			...
			data['rooms'].forEach((room) => {
				// console.log(room)
				ul.append('<li onclick="selectRoom(this)">'+
                                room.roomName +'<div class="green-dot"></div></li>');
		});
		})
	}, 1500);
...
generateRoomId = () => {
	return ((Math.floor(Math.random() * 100000 ))+ generateId(20) + (Math.floor(Math.random() * 10000)))
}
dec2hex = (dec) => {
