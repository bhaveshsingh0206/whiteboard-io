...
const user = {
	...
	designation: document.getElementById('designation').textContent.toLowerCase()
};
...
function startProgress() {
	...
	if (!progressBar.classList.contains('progress-infinite')) {
		document.querySelector('#progress-bar').classList.add('progress-infinite');
	}
}
...
function endProgress() {
	const progressBar = document.querySelector('#progress-bar');
	if (progressBar.classList.contains('progress-infinite')) {
		document.querySelector('#progress-bar').classList.remove('progress-infinite');
	}
}
...
async function getClassInformation() {
	const classId = document.querySelector('#username').getAttribute('data-classid');
	try {
...
async function getPostsByClass(classid) {
	try {
		const response = await fetch(`/post/api/dashboard/filter/${classid}/${user.id}`);
		const data = await response.json();
...
async function getPosts() {
	try {
		const response = await fetch(`/post/api/dashboard/${user.designation}/${user.id}`);
		const data = await response.json();
		const posts = data.posts;
		if (posts.length == 0) {
			globals.container.innerHTML += `<div class="no-posts">No posts found</div>`;
...
async function getNotifications() {
	try {
		const response = await fetch(`/notification/api/get/all/${user.username}`);
		const data = await response.json();
...
function setPosts(posts) {
	...
	for (i = 0; i < posts.length; i++) {
		...
		let postHTML = `<div class="post-container ${padding}" data-id="${post._id}">
					<div class="post">
						<div class="post-header">
							...
							<div class="post-download" id="${post._id}" onclick="getId(this.id)"><p class="header-3">Download</p></div>
						</div>
						<div class="post-body">
	...
	for (var i = 0; i < posts.length; i++) {
		...
		editor.session.setValue(formatCode(post.code, post.input, post.output, post.extension));
		if (post.extension == 'c') {
...
function setNotifications(notifications) {
	...
	for (let i = 0; i < notifications.length; i++) {
		...
		const today = new Date();
		let notificationHTML = `<div class="post-container ${padding} data-id="${notification._id}">
					<div class="post">
						<div class="post-header">
							<div class="post-header-left">
							...
							<div class="post-reply" id="${notification._id}" onclick="getNotificationId(this.id)">
...
function formatDate(ISODate, today) {
	...
	const time = `${formatTime(date.getHours())}:${formatTime(date.getMinutes())}`; // Structuring the time HH:mm and adding leading zeros
	const months = [
		'Jan',
		'Feb',
		'Mar',
...
async function deletePost(buttonId) {
	...
	if (confirm('Are you sure you want to delete?')) {
		try {
			const response = await fetch(`/post/api/dashboard/delete/${postId}`, { method: 'DELETE' });
			const data = await response.json();
			alert(data.message);
			...
			communityPosts.innerHTML = parseInt(communityPosts.textContent, 10) - 1;
...
async function deleteNotification(buttonId) {
	...
	if (confirm('Are you sure you want to delete?')) {
		try {
			const response = await fetch(`/notification/api/dashboard/delete/${notificationId}`, { method: 'DELETE' });
			const data = await response.json();
			alert(data.message);
...
async function getNotificationId(notificationId) {
	try {
		const response = await fetch(`/notification/api/get/specific/${notificationId}`);
		const data = await response.json();
		const notification = data.notification;
		document.querySelector('#notif-to-address').value = notification.fromusername;
		document.querySelector('#notif-code').value = notification.code;
		document.querySelector('#notif-comments').value = `RE: ${notification.comment}\n`;
	} catch (err) {
		console.log(err);
	}
}