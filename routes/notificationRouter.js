...
router.get('/api/get/all/:username', async (req, res) => {
	try {
		const notifications = await Notif.find({ tousername: req.params.username }).sort({ date: -1 });
...
router.delete('/api/dashboard/delete/:notificationid', async (req, res) => {
	const message = await Notif.findByIdAndDelete(req.params.notificationid).then(
