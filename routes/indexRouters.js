...
router.post('/register', (req, res) => {
	...
	var class_id = '';
	Class.findOne({ year: req.body.class_name, batch_name: req.body.batch }, (err, clas) => {
		...
		if (err) {
			...
			if (req.body.password === req.body.confirm_password) {
				...
				Student.register(newStudent, req.body.password, async (err, user) => {
					if (err) {
						console.log(err);
...
router.post('/getforgetpasswordlink', (req, res) => {
	...
	var mailOptions = {
		...
		text:'Open this link to reset password  https://callback-vtzhu.run-us-west2.goorm.io/forgetpassword/' +
			token
	};
...
router.get('/notif/:username', async (req, res) => {
	try {
		const notifs = await Notif.find({ tousername: req.params.username }).sort({ date: -1 });
