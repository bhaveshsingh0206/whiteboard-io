...
router.get('/api/dashboard/:designation/:userid', async (req, res) => {
	try {
		if (req.params.designation == 'teacher') {
			const postsObj = await Post.find({ teacher_id: req.params.userid }).sort({ date: -1 });
			const posts = [];
			...
			const postsObj = await Post.find({ classId: classObj._id }).sort({ date: -1 });
			const posts = [];
			for (let postObj of postsObj) {
				let post = postObj.toObject();
				const teacherObj = await Teacher.findById(post.teacher_id);
...
router.get('/api/dashboard/statistics/teacher/:userid/:username', async (req, res) => {
	try {
		const yourPosts = await Post.countDocuments({ teacher_id: req.params.userid });
		const allPosts = await Post.countDocuments();
		const myNotifications = await Notif.countDocuments({ tousername: req.params.username });
		res
			.status(200)
			.json({ yourPosts: yourPosts, allPosts: allPosts, myNotifications: myNotifications });
	} catch (err) {
		console.log(err);
		res.status(400).json({ error: err });
	}
...
router.get('/api/dashboard/statistics/student/:classid/:username', async (req, res) => {
	try {
		const batchPosts = await Post.countDocuments({ classId: req.params.classid });
		const allPosts = await Post.countDocuments();
		const myNotifications = await Notif.countDocuments({ tousername: req.params.username });
		res
			.status(200)
			.json({ batchPosts: batchPosts, allPosts: allPosts, myNotifications: myNotifications });
	} catch (err) {
		console.log(err);
		res.status(400).json({ error: err });
	}
